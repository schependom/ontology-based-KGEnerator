...
from data_structures import KnowledgeGraph, Constraint, Triple, Membership
            ...
            if prop_name in self.domains:
                ...
                for domain_cls in required_domains:
                    if domain_cls not in subj_classes:
                        errors.append(
                            f"Schema Violation: Subject {subj_name} of {prop_name} is not type {domain_cls}"
                        )
            ...
            if prop_name in self.ranges:
                ...
                for range_cls in required_ranges:
                    if range_cls not in obj_classes:
                        errors.append(
                            f"Schema Violation: Object {obj_name} of {prop_name} is not type {range_cls}"
                        )
